...
// Ergebnis!). Deshalb wird das per assert() explizit abgesichert,
// statt es stillschweigend potenziell abstürzen zu lassen.
//
...
namespace SortAlgorithms
{
    ...
    void radixSortImpl(std::vector<std::int32_t>& arr, const StepCallback& cb, LiveMetrics& m)
    {
        ...
        assert(std::ranges::all_of(arr, [](std::int32_t v) { return v >= 0; }) &&
